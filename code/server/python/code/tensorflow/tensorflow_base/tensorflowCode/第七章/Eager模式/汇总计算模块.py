...
#
# 求均值
# tf.keras.metrics() 指标计算模块
# keras.metrics中有两个api函数可以简化准确率acc和损失值loss的计算。其分别是metrics.Accuracy( )和metrics.Mean( )。
m = tf.keras.metrics.Mean('acc')

print('m(10) 每一次调用都在计算均值  =', m(10))
print('m(20) ==', m(20))
# 返回均值
print('返回均值  m.result() ==>', m.result())
print(' 可以调用多个数据 m([30,40]) ==', m([30,40]))
# 重置对象
m.reset_states()
# --------------------------------------------------------
...
# SparseCategoricalAccuracy方法 可以自动从 model(features) 选出最大值，然后与 labels 进行比较
print('a(labels, model(features))', a(labels, model(features)))


# 成功 在 32 个值中 预测 对 4个值， 所以为 4、32 =0.125
# prediction_argmax tf.Tensor([6 6 6 6 6 6 6 6 6 6 1 1 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6], shape=(32,), dtype=int64)
# labels == tf.Tensor([6 4 1 4 2 4 3 6 9 0 1 2 0 5 9 2 9 1 3 2 9 9 9 8 6 8 2 3 1 1 8 3], shape=(32,), dtype=int64)