...
def load_preprosess_image(image_path):
    ...
    # img_data_png = tf.image.convert_image_dtype(image_tensor, dtype=tf.uint8)
    # print('img_data_png ---', img_data_png)
    # 转成 float32 格式
    ...
    # print('image_tensor_case', image_tensor_case)
    print('image_tensor_case_normalize', image_tensor_case_normalize.shape)
    print('image_tensor_case_normalize', image_tensor_case_normalize.numpy())
    return image_tensor_case_normalize

# 猜测 imshow 若是读取 tf.uint8 数据 那么rgb的取值范围为 0~255， 若是读取 float32 取值 为0~1
