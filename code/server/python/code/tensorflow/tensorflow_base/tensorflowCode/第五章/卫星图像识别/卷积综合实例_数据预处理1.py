...
# 循环三次 list(range(3)) = [0, 1, 2]
for n in range(3):
    # random.choice 随机取值
    image_index = random.choice(range(len(all_image_path_list)))
    print('image_index', all_image_path_str[image_index])

    # lena = mpimg.imread(all_image_path_str[image_index])
    # print('lena', lena)
    # plt.show()

# ------------------------------
# 汇总成  load_preprosess_image 函数
# # tf 读取图片 tf.io.read_file 显示编码形式
# image_raw = tf.io.read_file(all_image_path_str[0])
# # 解码图片
...
for image_label_index in label_dataset.take(10):
