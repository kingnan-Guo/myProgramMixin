...
#
all_test_image_path = data_root.glob('./test/*/*.jpg')
...
test_image_label = [int(P.split('/')[15] == 'cat') for P in all_test_image_path_str_list]
...
# test 数据预处理 ------
test_image_dataset = tf.data.Dataset.from_tensor_slices((all_test_image_path_str_list, test_image_label))
# 循环处理 test_image_dataset 剪裁图像 并 归一化处理
test_image_dataset_map = test_image_dataset.map(load_preprosess_image, num_parallel_calls=AUTOTUNE)

test_count = len(all_train_image_path_list)
# 测试数据 不走shuffle
test_image_dataset_map_batch_size = train_image_dataset_map.batch(batch_size)
...
def test_step(model, images, labels):
    # 这两种方法都是 不会将 test的数据集 放到模型中进行预测
...
for epoch in range(num_epochs):
    ...
    # 测试数据集的
    for imgs_, labels_ in test_image_dataset_map_batch_size:
        test_step(model, imgs_, labels_)
    test_loss_result.append(epoch_loss_avg_test.result())
    test_acc_result.append(test_accuracy.result())


    print('Epoch: {};loss: {:.3f}, accuracy: {:3f}'.format(epoch + 1, epoch_loss_avg_test.result(), test_accuracy.result()))
    # 若没有重置状态，那么就 会计算所有数据的平均值
